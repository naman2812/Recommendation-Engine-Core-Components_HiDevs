...
*   **`tests/`**: A comprehensive `pytest` test suite with high coverage to verify all edge cases and calculations.
*   **`Dockerfile`**: Containerizes the application to ensure seamless deployment and environment consistency.
*   **`.github/workflows/ci.yml`**: A Continuous Integration pipeline that automatically runs the test suite upon pushing to GitHub.
...
To install dependencies and run the automated `pytest` suite with code coverage:
```bash
pip install -r requirements.txt
pytest --cov=. -v
```

To run via Docker (Deployment mode):
```bash
docker build -t recommendation-engine .
docker run recommendation-engine
```
